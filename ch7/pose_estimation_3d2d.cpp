...
/**
 * BA(bundle agjustment) in using G2O
 * @param point_3d points 3d coor
 * @param point_2d points 2d coor
 * @param K        camera Intrinsics matrix
 * @param pose     camera pose in sophus
 */
...
int main(int argc, char **argv)
{
  ...
  for (DMatch m : matches)
  {
    ...
    if (d == 0) // bad depth
      continue;
    float dd = d / 5000.0; // 深度图的像素值与实际深度的尺度
    Point2d p1 = pixel2cam(keypoints_1[m.queryIdx].pt, K); // 像素归一化坐标
    ...
    Sophus::SE3d &pose)
{
  ...
  double fx = K.at<double>(0, 0); // 相机内参
  double fy = K.at<double>(1, 1); // 相机内参
  double cx = K.at<double>(0, 2); // 相机内参
  double cy = K.at<double>(1, 2); // 相机内参
  ...
  for (int iter = 0; iter < iterations; iter++)
  {
    ...
    {
      Eigen::Vector3d pc = pose * points_3d[i]; // P'=(T*P[1:3])=(X',Y',Z')
    ...
    lastCost = cost; // BA by gauss-newton
    ...
    Sophus::SE3d &pose)
{
  ...
  vertex_pose->setId(0); // 对顶点进行编号，0可以写为任意整数，后面设置edge连接顶点时，必须与括号内的一致
