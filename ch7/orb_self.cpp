...
{
    // 加载图片
    cv::Mat first_image = cv::imread(first_file, 0);                    // 0表示返回一张灰度图
    cv::Mat second_image = cv::imread(second_file, 0);                  // 0表示返回一张灰度图
    assert(first_image.data != nullptr && second_image.data != nullptr);// 断言函数，条件为假则程序停止执行

    // 检测FAST特征点， 阈值threshold=40
    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
    vector<cv::KeyPoint> keypoints1;
    cv::FAST(first_image, keypoints1, 40);                              // 40是阈值 调用FAST计算特征点
    vector<DescType> descriptor1;                                       // 计算第一张图片的描述子
    ComputeORB(first_image, keypoints1, descriptor1);

    // 第二张图片同理
    vector<cv::KeyPoint> keypoints2;
    vector<DescType> descriptor2;
    cv::FAST(second_image, keypoints2, 40);                             // 40是阈值 调用FAST计算特征点
...
// 每一行代表一对点的坐标，根据BRIEF第二步判断这一行的二进制是0还是1，先找图像中心
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors)
{
    const int half_patch_size = 8;      // 计算特征点方向时选取的图像块， 16 * 16
    const int half_boundary = 16;       // 计算描述子时在32 * 32 的图像中选
    int bad_points = 0;                 // 在32*32的区域选两个点比较，所选的点超出图像范围的
    // 遍历所有的角点
    for(auto &kp : keypoints)
    {
        ...
        for (int dx = -half_patch_size; dx <half_patch_size; ++dx)
        {
            ...
            {
                uchar pixel = img.at<uchar>(kp.pt.y + dy, kp.pt.x + dx);    // 获取像素
                m10 += dx * pixel;
                m01 += dy * pixel;
            }
        }
        ...
        float m_sqrt = sqrt(m01 * m01 + m10 * m10) + 1e-18;     // avoid divide by zero(图像块全黑)
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches)
{
    ...
    for (size_t i1 = 0; i1 < desc1.size(); ++i1)
    {
        ...
        for (size_t i2 = 0; i2 < desc2.size(); ++i2)
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                distance += _mm_popcnt_u32(desc1[i1][k] ^ desc2[i2][k]);    // 计算Hamming距离
            }
